...
class ColmapParser:
    ...
    def __init__(
        ...
        # load in 3D points
        #ply_path = Path(colmap_dir / "points3D.ply") 
        #if ply_path.exists():
         #   ply = o3d.io.read_point_cloud(str(ply_path))
          #  points = np.asarray(ply.points, dtype=np.float32)
        ...
        # PRISM3D: Check if the colmap outputs are estimated on downscaled data.
    ...
    "-i", str(path),  # Ensure this is a string
    "-q:v", "2",
    "-vf", f"scale={w_scaled}:{h_scaled}{nn_flag}",
    str(path_out),  # Also ensure this is a string
]
