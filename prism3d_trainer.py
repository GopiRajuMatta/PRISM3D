...
class DeblurRunner(Runner):
    ...
    def train(self):
        ...
        use_event = False#args.use_event
        if use_event:
        # load event
            event_maps = torch.load(os.path.join(cfg.data_dir, "events.pt")).to(device).float()
        # rgb2grey
        #event_maps = event_maps.view(event_maps.shape[0],4, 260,346)
            event_maps = event_maps.view(event_maps.shape[0],4, 540,800)
        ...
        for step in pbar:
            ...
            #event_loss = event_loss_call_linlog(colors, events, 4, cfg.data_factor) * 0.005
