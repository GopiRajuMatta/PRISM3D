...
from prism3d.camera_optimizer import Prism3DCameraOptimizer, Prism3DCameraOptimizerConfig
...
from base_trainer import Config, Runner, create_splats_with_optimizers
...
class DeblurConfig(Config):
    ...
    data_dir: str = "/datasets/prism3d/data/bad-nerf-gtK-colmap-nvs/blurtanabata"
    ...
    # PRISM3D: Bundle adjusted deblur camera optimizer
    camera_optimizer: Prism3DCameraOptimizerConfig = field(
        default_factory=lambda: Prism3DCameraOptimizerConfig(
            mode="bezier",
            num_virtual_views=14,
        )
    )
    ...
    prism3d_post_init_complete: bool = False

    def __post_init__(self):
        if not self.prism3d_post_init_complete:
            self.prism3d_post_init_complete = True
...
class DeblurRunner(Runner):
    ...
    def __init__(self, local_rank: int, world_rank, world_size: int, cfg: DeblurConfig) -> None:
        ...
        self.camera_optimizer: Prism3DCameraOptimizer = self.cfg.camera_optimizer.setup(
    ...
    def train(self):
        ...
        use_event = True#args.use_event
        print("cfg.data_dir",cfg.data_dir)
        if use_event:
        # load event
            event_maps = torch.load(os.path.join(cfg.data_dir, "events_bins_13.pt")).to(device).float()
        # rgb2grey
        #event_maps = event_maps.view(event_maps.shape[0],4, 260,346)
            event_maps = event_maps.view(event_maps.shape[0],13, 540,800)
        ...
        for step in pbar:
            ...
            #print(data.keys())
            #print("image_id",image_ids,data["colmap_image_id"])
            # forward
            #print(Ks)
            ...
            #print("data[image_name]",int(data["image_name"][0].split(".")[0]), data["colmap_image_id"])
            # PRISM3D: average the virtual views
            #print("colors.shape",colors.shape)
            cfg.event_loss = True
            if cfg.event_loss:
                events = event_maps[int(data["colmap_image_id"]),:,:,:]
            #print("events.shape",events.shape)
                #event_loss = event_loss_call_linlog_normalized_random_consecutive(colors, events, cfg.data_factor,  13) * 1.0
                event_loss = event_loss_call_linlog_normalized(colors, events, cfg.data_factor, 13)*0.005
...
if __name__ == "__main__":
    ...
    ```bash
    # Single GPU training
    CUDA_VISIBLE_DEVICES=0 python base_trainer.py default
    # Distributed training on 4 GPUs: Effectively 4x batch size so run 4x less steps.
    CUDA_VISIBLE_DEVICES=0,1,2,3 python base_trainer.py default --steps_scaler 0.25
    """
